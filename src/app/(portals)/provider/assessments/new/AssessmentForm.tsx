...
export function AssessmentForm({ patientId, providerId, isCaseManager = true }: AssessmentFormProps) {
  ...
  const [medications, setMedications] = React.useState<MedicationDraft[]>([{ name: "", dosage: "", frequency: "" }])
  ...
  const handleSignatureSave = (signatureBase64: string) => {
    ...
    startTransition(async () => {
      ...
      if (invalidMedication) {
        toast.error("Each medication entry must include name, dosage, and frequency.")
        return
      }
              ...
              <div className="space-y-3">
                {medications.map((medication, index) => (
                  <div
                    ...
                    className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto] rounded-xl border border-slate-200 p-3"
                  >
                    <Input
