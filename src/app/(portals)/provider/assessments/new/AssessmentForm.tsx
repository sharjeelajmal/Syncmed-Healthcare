...
type QuestionType = "text" | "textarea" | "single" | "multi" | "date"
...
const memberInfoFields: MemberInfoField[] = [
...
const step2Sections: SectionDefinition[] = [
  {
...
const step3Sections: SectionDefinition[] = [
  {
...
const step4Sections: SectionDefinition[] = [
  {
...
const step5Questions: QuestionDefinition[] = [
...
function deriveRiskLevel(totalRiskScore: number): "LOW" | "MODERATE" | "HIGH" {
...
export function AssessmentForm({ patientId, providerId, isCaseManager = true }: AssessmentFormProps) {
  ...
  const [calculatedBmi, setCalculatedBmi] = React.useState(0)
  const [bmiCategory, setBmiCategory] = React.useState("N/A")
  ...
  React.useEffect(() => {
    ...
    const nextBmi = weight / (height / 100) ** 2
    const rounded = Number(nextBmi.toFixed(1))
    setCalculatedBmi(rounded)
    setBmiCategory(getBmiCategory(rounded))
  }, [weightKg, heightCm])
    ...
    <>
        ...
        {currentStep === 4 ? (
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 space-y-4">
              ...
              <p className="text-sm text-slate-600">Enter weight in kg and height in cm to auto-calculate BMI and category.</p>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-slate-600">Weight (kg)</Label>
                  <Input value={weightKg} onChange={(e) => setWeightKg(e.target.value)} className="h-11 border-slate-200 text-slate-700" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-slate-600">Height (cm)</Label>
                  <Input value={heightCm} onChange={(e) => setHeightCm(e.target.value)} className="h-11 border-slate-200 text-slate-700" />
                </div>
                <div className="space-y-2">
      ...
      <div className="sticky bottom-3 mt-8 rounded-2xl border border-slate-200 bg-white/95 backdrop-blur p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <Badge className="bg-slate-100 text-slate-700 border-slate-200 px-3 py-1.5 w-fit">
          Step {currentStep + 1} / {WIZARD_STEPS.length}: {WIZARD_STEPS[currentStep]}
        </Badge>
