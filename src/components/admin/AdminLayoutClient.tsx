...
export function AdminLayoutClient({
  ...
  // Use DB user if available, fallback to session, then default
  const displayName = user?.firstName ? `${user.firstName} ${user.lastName}` : (session?.user?.name || 'System Administrator')
  const displayImage = user?.image || (session?.user as any)?.image
        ...
        <div className="container mx-auto flex h-16 items-center justify-between px-4 sm:px-8">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex flex-col items-end mr-2 text-right">
              <span className="text-xs font-black text-[#67BA2E] uppercase tracking-widest">ADMIN</span>
              <span className="text-[10px] font-bold text-slate-400 tracking-tight opacity-70">{displayName.trim() || 'System Administrator'}</span>
            </div>
            
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center border border-slate-200 shadow-sm cursor-pointer overflow-hidden hover:bg-slate-100 transition-all">
                   {displayImage ? (
